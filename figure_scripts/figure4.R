...
#Setting up the data
GSE99791_df <- data.frame(results = c(as.numeric(Results_GSE99791[["cerebellum"]][["sig_results"]]),
                                          as.numeric(Results_GSE99791[["cerebellum"]][["all_results"]])), 
                              age = rep(as.numeric(Results_GSE99791[["cerebellum"]][["age"]]), 2),
...
GSE99791_df_text <- data.frame(
  ...
  p = c(paste0("p = ", signif( cor.test(as.numeric(Results_GSE99791[["cerebellum"]][["sig_results"]]), Results_GSE99791[["cerebellum"]][["age"]], method = "spearman", exact = F)$p.val, 2 ) ),
        paste0("p = ", signif( cor.test(as.numeric(Results_GSE99791[["cerebellum"]][["all_results"]]), Results_GSE99791[["cerebellum"]][["age"]], method = "spearman", exact = F)$p.val, 2 ) )),
...
figure4 <- ggplot(data = GSE99791_df, aes(age, results)) +
  ...
  ggtitle(expression(paste(italic("M. musculus")," astrocytes from cerebellum")))+
  xlab("Age (months)")+
...
ggsave("figure4-astrocyte-transcriptome-cons-vs-age.tiff", plot = figure4, device = "tiff",
       path = "results_graphs/", type = "cairo",
       scale = 1, width = 10, height = 5, units = "in", dpi = 300)
ggsave("figure4-astrocyte-transcriptome-cons-vs-age.pdf", plot = figure4, device = "pdf",
       path = "results_graphs/",
       scale = 1, width = 10, height = 5, units = "in", dpi = 300)
