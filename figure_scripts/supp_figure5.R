...
supp_fig_5 <- p1 + p2 +  p3 + p4 + plot_layout(ncol = 2, width = c(3,1), guides = "collect")& 
  #  theme(legend.position = 'top')&
  plot_annotation(tag_levels = c('A', 'B'))& 
  theme(plot.tag = element_text(size = 14, face = "bold"))+
  theme(plot.margin = unit(c(0.6, 0.6, 0.1, 0.1), "cm"))

ggsave("suppFig5-celltype-diff-conservation.png", plot = supp_fig_5, device = png,
       path = "results_graphs/", type = "cairo",
       scale = 1, units = "cm", width = 42, height = 38, dpi = 300)
ggsave("suppFig5-celltype-diff-conservation.tiff", plot = supp_fig_5, device = "tiff",
       path = "results_graphs/", type = "cairo",
       scale = 1, units = "cm", width = 42, height = 38, dpi = 300)
ggsave("suppFig5-celltype-diff-conservation.pdf", plot = supp_fig_5, device = "pdf",
       path = "results_graphs/", 
       scale = 1, units = "cm", width = 42, height = 38, dpi = 300)
